...
from mlflow.pyfunc import ResponsesAgent  # type: ignore
...
class ArxivAgent(ResponsesAgent):
    ...
    def call_llm(
        ...
        with warnings.catch_warnings():
            ...
            stream = self.model_serving_client.chat.completions.create(
                ...
                messages=to_chat_completions_input(messages),  # type: ignore
                tools=self.get_tool_specs(),  # type: ignore
                stream=True,
            )  # type: ignore
    ...
    def handle_tool_call(self, tool_call: dict[str, Any], messages: list[dict[str, Any]]) -> ResponsesAgentStreamEvent:
        ...
        return ResponsesAgentStreamEvent(
            ...
            item=tool_call_output,  # type: ignore
        )
    ...
    def save_memory(self, session_id: str, messages: list[dict[str, Any]]) -> None:
        ...
        self.memory.save_messages(session_id, messages)  # type: ignore
    ...
    def _extract_output_items(
        ...
        for e in events:
            if e.type != "response.output_item.done":
                continue
            item = e.item if isinstance(e.item, dict) else e.item.model_dump()  # type: ignore
    ...
    def _run_tool_loop(
        ...
        for _ in range(max_iter):
            ...
            events.append(
                ResponsesAgentStreamEvent(
                    type="response.output_item.done",
                    item=self.create_text_output_item(  # type: ignore
    ...
    def predict(self, request: ResponsesAgentRequest) -> ResponsesAgentResponse:
        ...
        return ResponsesAgentResponse(
            output=self._extract_output_items(events),  # type: ignore
...
) -> mlflow.entities.model_registry.RegisteredModel:  # type: ignore
    """
