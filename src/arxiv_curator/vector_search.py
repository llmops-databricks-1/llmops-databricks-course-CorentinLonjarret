...
class VectorSearchManager:
    ...
    def __init__(
        ...
        self.index_name = f"{self.catalog}.{self.schema}.{self.index_table}"
    ...
    def create_endpoint_if_not_exists(self) -> None:
        ...
        if not endpoint_exists:
            ...
            self.client.create_endpoint_and_wait(
                name=self.endpoint_name, endpoint_type="STANDARD", usage_policy_id=self.usage_policy_id
            )
    ...
    def sync_index(self) -> None:
        ...
        for attempt in range(1, max_retries + 1):
            try:
                logger.info(f"Attempt {attempt}/{max_retries} — Syncing vector search index: {self.index_name}")
                index.sync()
                logger.info("✓ Index sync triggered")
                break
            except Exception as e:
                if "not ready yet" in str(e) and attempt < max_retries:
                    logger.warning(f"Endpoint not ready yet. Retrying in {retry_interval_sec}s...")
    ...
    def search(self, query: str, num_results: int = 5, filters: dict | None = None) -> dict:
        ...
        results = index.similarity_search(
            query_text=query, columns=["id", "text", "metadata"], num_results=num_results, filters=filters
        )
        return results
