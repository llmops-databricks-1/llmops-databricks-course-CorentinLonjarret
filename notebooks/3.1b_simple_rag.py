...
Answer the question based on the provided context from research papers.
...
def rag_query(question: str, num_docs: int = 5) -> dict:
    ...
    return {
        "question": question,
        "answer": answer,
        "sources": [{"title": doc["title"], "arxiv_id": doc["arxiv_id"]} for doc in documents],
    }
...
class SimpleRAG:
    ...
    def retrieve(self, query: str, num_results: int = 5) -> list[dict]:
        ...
        results = index.similarity_search(
            query_text=query, columns=["text", "title", "arxiv_id"], num_results=num_results, query_type="hybrid"
        )
    ...
    def chat(self, question: str, num_docs: int = 3) -> str:
        ...
        system_message = f"""You are a helpful research assistant.
    Use the following context from research papers to answer questions.
