...
test_request = ResponsesAgentRequest(
    input=[{"role": "user", "content": "Find papers about transformers and attention mechanisms"}],  # type: ignore
...
logger.info(response.output[-1].content)  # type: ignore
...
request1 = ResponsesAgentRequest(
    input=[{"role": "user", "content": "Calculate 100 plus 50"}],  # type: ignore
...
logger.info(f"Agent: {response1.output[-1].content}")  # type: ignore

# Turn 2 (with context)
request2 = ResponsesAgentRequest(
    input=[
        {"role": "user", "content": "Calculate 100 plus 50"},
        {"role": "assistant", "content": response1.output[-1].content},  # type: ignore
...
logger.info(f"Agent: {response2.output[-1].content}")  # type: ignore
...
if len(session_traces_df) > 0:
    logger.info(f"Available columns: {list(session_traces_df.columns)}")  # type: ignore

    # Select only scalar columns to avoid Arrow conversion errors
    simple_cols = []
    for col in session_traces_df.columns:  # type: ignore
        if col not in ["request", "response", "spans", "inputs", "outputs"]:
            simple_cols.append(col)

    if simple_cols:
        display(session_traces_df[simple_cols].head())  # type: ignore  # noqa: F821
    else:
        logger.info(str(session_traces_df.info()))  # type: ignore
else:
...
if len(recent_traces_df) > 0:
    ...
    if "execution_time_ms" in recent_traces_df.columns:  # type: ignore
        durations = recent_traces_df["execution_time_ms"].dropna()  # type: ignore
        if len(durations) > 0:
    ...
    if "status" in recent_traces_df.columns:  # type: ignore
        logger.info("By Status:")
        status_counts = recent_traces_df["status"].value_counts()  # type: ignore
        for status, count in status_counts.items():
            logger.info(f"  {status}: {count}")

    # Show sample of traces (select only simple columns to avoid Arrow conversion issues)
    logger.info("Sample Traces:")
    logger.info(f"Available columns: {list(recent_traces_df.columns)}")  # type: ignore

    # Select only scalar columns for display
    simple_cols = []
    for col in recent_traces_df.columns:  # type: ignore
        # Skip complex object columns that cause Arrow conversion errors
        if col not in ["request", "response", "spans", "inputs", "outputs"]:
            simple_cols.append(col)

    if simple_cols:
        display(recent_traces_df[simple_cols].head())  # type: ignore  # noqa: F821
    else:
        # Fallback: just show the info
        logger.info(str(recent_traces_df.info()))  # type: ignore
...
if len(recent_traces_df) > 0:
    ...
    trace = recent_traces_df.iloc[0]  # type: ignore
