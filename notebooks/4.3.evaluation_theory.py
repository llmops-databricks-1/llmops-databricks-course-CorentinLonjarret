...
# Evaluate
results = mlflow.genai.evaluate(data=test_data, scorers=[polite_tone_guideline])  # type: ignore
...
judge_test_data = [
    ...
    {
        "inputs": {"question": "What is machine learning?"},
        "outputs": "Machine learning is a subset of AI where algorithms learn patterns from data to make predictions or decisions without being explicitly programmed.",  # noqa: E501
    },
]

# Evaluate
judge_results = mlflow.genai.evaluate(data=judge_test_data, scorers=[quality_judge])  # type: ignore
...
@mlflow.genai.scorer  # type: ignore
def word_count_check(outputs: list) -> bool:  # noqa: F811
...
custom_results = mlflow.genai.evaluate(  # type: ignore
    data=custom_test_data,
    scorers=[word_count_check, has_code_example, response_length_score],  # type: ignore
)
...
sentiment_judge = make_judge(
    ...
    instructions=(
        "Analyze the sentiment of the response in {{ outputs }}. Classify as: 'positive', 'neutral', or 'negative'"
    ),
...
comprehensive_test_data = [
    {
        ...
        "outputs": "Transformers are a neural network architecture that uses self-attention mechanisms to process sequential data. They've revolutionized NLP by enabling models like BERT and GPT.",  # noqa: E501
    },
]

# Evaluate with all scorers
comprehensive_results = mlflow.genai.evaluate(data=comprehensive_test_data, scorers=all_scorers)  # type: ignore

logger.info("Comprehensive Evaluation Results:")
logger.info("=" * 80)
comprehensive_results  # type: ignore # noqa: B018
