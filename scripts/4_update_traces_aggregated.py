...
from arxiv_curator.utils import get_job_parameter, set_mlflow_tracking_uri
...
experiment_id = mlflow.get_experiment_by_name(cfg.experiment_name).experiment_id  # type: ignore
logger.info(f"Using MLflow experiment: {cfg.experiment_name} (ID: {experiment_id})")


def log_feedback_if_trace_exists(*, trace_id: str, name: str, value: str | bool) -> None:
    try:
        mlflow.log_feedback(
            trace_id=trace_id,
            name=name,
        ...
        if getattr(exc, "error_code", None) == "NOT_FOUND" or "not found" in str(exc).lower():
            logger.warning(f"Skipping feedback '{name}' for missing trace_id={trace_id}: {exc}")
        else:
            raise
...
logger.info(f"Reading traces from table: {traces_table}")
# Target view: aggregated metrics + quality scores per trace
aggregated_view = f"{catalog}.{schema}.{cfg.aggregated_view}"
logger.info(f"Aggregated view to create: {aggregated_view}")

# For reference, the serving endpoint name is:
endpoint_name = f"arxiv-agent-endpoint-{env}-course-corentin"
logger.info(f"Processing traces for endpoint: {endpoint_name}")
...
wc_result = mlflow.genai.evaluate(  # type: ignore
...
for trace_id, assessments in zip(
    eval_pdf["trace_id"],
    wc_result.result_df["assessments"],  # type: ignore
    strict=True,
):
...
llm_result = mlflow.genai.evaluate(  # type: ignore
...
for trace_id, assessments in zip(
    ...
    llm_result.result_df["assessments"],  # type: ignore
    strict=True,
):
